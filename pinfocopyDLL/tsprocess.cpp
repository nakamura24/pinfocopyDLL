...
int getPid(const unsigned char* buf)							// bufは0x47ヘッダの場所
{
...
int getSectionLength(const unsigned char* buf)						// bufはTableIDの場所
{
...
int parsePat(const unsigned char *buf, int *pmtpid)
{
	...
	while( i < (seclen - 1) )
	{
		...
		if(service_id != 0) {								// service_id == 0 は NIT
...
void parsePmt(const unsigned char *buf, int *pcrpid, int *videopid, int *rmpid, int *rmnum, const BOOL bRmCap, const BOOL bRmDat)
{
	...
	if(bRmCap || bRmDat) {
		rmpid[rmcount++] = 0x1FFF;															// NULLパケットPID追加
		qsort(rmpid, rmcount, sizeof(int), compareForPidTable);								// 削除用PIDの昇順ソート qsortに変更した
	}
...
int compareForPidTable(const void *item1, const void *item2)								// 削除PIDテーブル qsort, bsearch 用関数
{
...
__int64 getPcrValue(const unsigned char *buf)
{
	...
	const unsigned char	*pcr = buf + 6;				// pcr_clock_reference_baseの先頭
